...
def run_vg2(folderpath, do_log, recenter, smoothing_bw, stiffness, vcenter, vwidth1, vwidth2):
        ...
        if filename[-3:] == 'txt':
            ...
            if do_log:
                dfxl = pd.concat([dfxl, pd.DataFrame(
                       [concxl, replicatexl, vg_df["V"], vg_df["I"], vg_df["logI"], vg_df["smoothed"], vg_df["detilted"]]).transpose()])
            else:
                dfxl = pd.concat([dfxl, pd.DataFrame(
            ...
            if peak_v == None:
                peak_v=0
            signal_lst.append([filename, round(peak_signal,2), round(peak_v,3), round(vcentershoulder,3), ph]) #add text filename & peak signal to signal list
    ...
    conc_df.to_excel(stats_str, index=False, header=["conc","average","std","CV","T-Statistic","avg peak", "std peak"])
    signal_df.to_excel(signal_str, index=False, header=["file", "signal","peak V", "vcenter", "PH"]) #save signal list to excel
    if do_log:
        dfxl.to_excel(dataframe_str, index=False, header=["conc","replicate","V","I","logI","smoothed","detilted"])
    else:
