...
def analyze(dir_path, peak_feat, pv_max):
...
if __name__ == '__main__':
    ...
    peak_ftr_dict = {2:'height', 3:'area', 4:'parea', 5:'absarea'}
    pv_max_str_dict = {1.10:'110', 1.15:'115'}

    for peak_feat in [2, 3, 4, 5]:
        for pv_max in [1.10, 1.15]:
            dir_path = f'C:/Users/jeongsu/Downloads/MLdataset_{peak_ftr_dict[peak_feat]}{pv_max_str_dict[pv_max]}'
            if not os.path.exists(dir_path):
                shutil.copytree(org_dir, dir_path)

            analyze(dir_path, peak_feat, pv_max)